...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyJLPKg-zQqKj215PNXSPMklwaze5ZCphifnzpTLX_Uy6sTqnvKb3QG6wZYd-tqNiVC/exec'; // <-- REPLACE this
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // If your Apps Script allows CORS, you can omit mode:'no-cors'
      // Some deployments require 'no-cors' (response will be opaque).
      await fetch(GOOGLE_SCRIPT_URL, {
        method: 'POST',
        mode: 'no-cors', // safe choice for GitHub Pages -> Apps Script; change to 'cors' if your script returns CORS headers
        headers: {
      ...
      // We assume success (no-cors responses don't return usable status)
      setIsSubmitted(true);
      setFormData({ name: '', email: '', phone: '', company: '', message: '' });

      // keep success visible for 3s
      setTimeout(() => {
        setIsSubmitted(false);
      }, 3000);
    } catch (err) {
      console.error('Submit error', err);
      setErrorMsg('There was an error submitting the form. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className={`w-full bg-blue-600 text-white px-8 py-4 rounded-lg font-semibold hover:bg-blue-700 transition-all duration-300 hover:shadow-lg flex items-center justify-center space-x-2 ${isLoading ? 'opacity-70 cursor-wait' : ''
                        }`}
                    >
